...
def ExpensesFromOwn(request):
    if request.method == "GET":
        ...
        entries = ExpenseFromTheir.objects.all().order_by("date")
        ...
        body[date_timestamp]["children"].append(
            {
                ...
                "client_name": entry.client_name,
                "administrator": f"{entry.stquest.administrator.last_name} {entry.stquest.administrator.first_name}",
                "sent": entry.sent,
                "type": entry.type,
                "note": entry.note,
                "quest": {
